...
class GraphCourseQueryService(_GraphQueryService, CourseQueryService):
    ...
    def _graph_get_course_by_uri(self, *, course_uri: URIRef) -> Course:
        ...
        course_description = self.cache_graph.value(course_uri, CRS_NS["hasDescription"])
        ...
        special_tags = frozenset(st.value for st in self.cache_graph.objects(course_uri, CRS_NS["hasSpecialTag"]))
        required_prerequisites = frozenset(self._graph_get_course_by_uri(course_uri=prc_uri) for prc_uri in
                                           self.cache_graph.objects(course_uri, CRS_NS["hasRequiredPrerequisite"]))
        recommended_prerequisites = frozenset(self._graph_get_course_by_uri(course_uri=prc_uri) for prc_uri in
                                              self.cache_graph.objects(course_uri, CRS_NS["hasRecommendedPrerequisite"]))
        topics = tuple(tp.value for tp in self.cache_graph.objects(course_uri, CRS_NS["hasTopic"]))
        offered_semesters = tuple(sem.value for sem in self.cache_graph.objects(course_uri, CRS_NS["hasOfferedSemesters"]))
    ...
    def _graph_get_course_by_course_code_uri(self, *, course_code_uri: URIRef) -> Course:
        pass
